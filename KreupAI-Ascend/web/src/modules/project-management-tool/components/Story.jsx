...
const StoryComponent = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [userGroupsResponse, usersResponse, storyResponse] = await Promise.all([
        ...
        axios.get('https://agilebackendtest-ig6zd90q.b4a.run//api/users'),
        isEditMode ? axios.get(`https://agilebackendtest-ig6zd90q.b4a.run//api/stories/${id}`) : Promise.resolve({ data: {} }),
      ]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        if (isEditMode) {
          await axios.put(`https://agilebackendtest-ig6zd90q.b4a.run//api/stories/${id}`, values);
          showNotification('success', 'Story Updated', 'Your story has been updated successfully.');
        } else {
          const response = await axios.post('https://agilebackendtest-ig6zd90q.b4a.run//api/stories', values);
